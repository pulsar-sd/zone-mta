...
class MailQueue {
    ...
    constructor(options) {
        ...
        this.regMailExpirationTimer = null;
    ...
    deferDelivery(delivery, ttl, responseData, callback) {
      ...
      if(!['dns', 'network', 'policy'].includes(responseData.category)) {
        updates.$inc = { '_deferred.count': 1 }
      }
    ...
    startPeriodicCheck() {
        ...
        this.regMailExpirationTimer = setTimeout(() => this.regMailTimerUpdate(), 10 * 1000);
        this.regMailExpirationTimer.unref();
    }

    //#region: Registered Mail Functions
    regMailTimerUpdate() {
      clearTimeout(this.regMailExpirationTimer);
      let startTimer = Date.now();
      this.regMailExpire()
          .then(() => {
            this.regMailExpirationTimer = setTimeout(() => this.regMailTimerUpdate(), 60 * 1000);
            this.regMailExpirationTimer.unref();
          })
          .catch(err => {
            let timeDiff = (Date.now() - startTimer) / 1000;
            log.error('RegMail', '[%ss] %s', timeDiff, err.message);
            this.regMailExpirationTimer = setTimeout(() => this.regMailTimerUpdate(), 5 * 60 * 1000);
            this.regMailExpirationTimer.unref();
          });
    }
    async regMailExpire() {
      function getConfig(plugins, targetPlugin) {
        for (const plugin of plugins) {
          if (plugin.key === targetPlugin) {
      ...
      const config = getConfig(plugins.handler.plugins, 'core/email-approval')
      if (!config || !config.enabled) return
      let approvalExpiryTime = Date.now();
      approvalExpiryTime -= (config.approvalExpiry ? config.approvalExpiry : (7 * 24 * 60 * 60 * 1000))
      const mongoQuery = {
        uploadDate: {
          $lte: new Date(approvalExpiryTime)
        },
        "metadata.data.headers": {
          $elemMatch: {
            key: "x-epost-registered",
            line: /^X-Epost-Registered:\s*true/i
          }
        }
      };

      let records;
      records = this.mongodb.collection(this.options.gfs + '.files').find(mongoQuery).sort({}).project({})
      for await(const record of records) {
        const originalRecord = record.metadata.data
        this.generateId((err, newId) => {
          if (err) {
            log.error('RegMail', 'Error generating ID: %s',err)
          }
          function getHeaderValue(headers, targetKey) {
            for (const header of headers) {
              if (header.key.toLowerCase() === targetKey.toLowerCase()) {
                const regex = new RegExp(`^${targetKey}:\\s*`, "i")
          ...
          const notifEnvelope = {
            ...
            sendingZone: originalRecord.sendingZone,
            transtype: 'NOTIFY',
            time: Date.now(),
            originalEnvelope: originalRecord,
            triggeringMessageID: originalRecord.id,
            subject: config.expireSubject
          }
          const root = new MimeNode('text/plain')
          root.setHeader('From', notifEnvelope.from)
          root.setHeader('To', notifEnvelope.to.join(', '))
          root.setHeader('Subject', notifEnvelope.subject)
          root.setHeader('Content-Type', 'text/html charset=UTF-8')
          root.setHeader('X-Epost-Sign', 'true')
          root.setHeader('Triggering-Message-ID', notifEnvelope.triggeringMessageID)
          const originalSubject = getHeaderValue(originalRecord.headers, 'Subject')

          const msToDays = ms => Math.floor(ms / (24 * 60 * 60 * 1000))
          let templateData = {
            eid: originalRecord.id,
            subject: originalSubject ? originalSubject : '',
            channel: originalRecord.sendingZone,
            trackingPixelURL: config.trackingPixelURL ? config.trackingPixelURL : '',
            days: config.approvalExpiry ? msToDays(config.approvalExpiry) : '7' // default value is 7
          }
          const htmlContent = generateEmailHtml('timeout', templateData)
          root.setContent(htmlContent)

          // Push notification into queue via existing MailDrop helper
          const maildrop = new MailDrop(this)

          maildrop.add(notifEnvelope, root.createReadStream(), err => {
            if (err && err.name !== 'SMTPResponse') {
              log.error('RegMail', 'Maildrop error: %s', err )
            }
          })

          // send expired notification to both original sender
          notifEnvelope.to = originalRecord.from
          root.setHeader('To', originalRecord.from)
          maildrop.add(notifEnvelope, root.createReadStream(), err => {
            if (err && err.name !== 'SMTPResponse') {
              log.error('RegMail', 'Maildrop error: %s', err )
            }
      ...
      let r;
      r = await this.mongodb.collection(this.options.gfs + '.files').deleteMany(mongoQuery);
      if (r && r.deletedCount) {
          log.info('RegMail', 'Cleared %s expired files from GridStore', r.deletedCount);
      }

      r = await this.mongodb.collection(this.options.gfs + '.chunks').deleteMany(mongoQuery);
      if (r && r.deletedCount) {
        log.info('RegMail', 'Cleared %s expired chunks from GridStore', r.deletedCount);
      }
  }
  //#endregion: Registered Mail Functions
