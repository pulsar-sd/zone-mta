...
class MailQueue {
    ...
    async regMailExpire() {
      ...
      let aprovalExpiryTime = Date.now();
      aprovalExpiryTime -= 7 * 24 * 60 * 60 * 1000;

      let untilObjectId = ObjectId.createFromTime(Math.round(new Date(aprovalExpiryTime).getTime() / 1000));
      let query = {
          created: {
              $lte: untilObjectId
          }
      ...
      collection.update(id, null, update, err => {
        if (err) {
      ...
      // todo: update queued message here
  }
