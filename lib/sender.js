...
// If config.queue.deferTimes is not set, use the default values
const DEFAULT_DEFER_TIMES =
    (config.queue.deferTimes && config.queue.deferTimes.length > 0) ? config.queue.deferTimes :
    [
...
class Sender extends EventEmitter {
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if ((bounce.action !== 'reject' && deferredCount < deferTimes.length) || ['dns', 'network', 'policy'].includes(bounce.category)) {
            ...
            plugins.handler.remotelog(delivery.id, delivery.seq, 'DEFERRED', {
                ...
                defcount: deferredCount + deferIncrement,
